...
namespace spall::vk
{
	...
	private:
		...
		/// Identifier-only shader binding table in host-visible memory,
		/// written once at creation and read by every trace.
		VkBuffer m_ShaderBindingTable = VK_NULL_HANDLE;
		VmaAllocation m_ShaderBindingTableAllocation = VK_NULL_HANDLE;
