...
static TestDevice requireRayQueryDevice(
	void)
{
	TestDevice testDevice = requireDevice(spall::RenderBackendType::Vulkan);
...
static std::array<RayResult, 2> traceScene(
	...
	RayTracingScene& scene,
	bool update = false,
	std::span<const std::uint32_t> bytecode = RayQueryComputeSpirv,
...
{
	const TestDevice testDevice = requireRayQueryDevice();
	...
	"[vulkan][GPU][raytracing]")
{
	...
	spall::Resource<spall::IShader> rayGenShader = device.pipelines().createShader(
		...
		RayTracingPipelineLibrarySpirv);
	...
	"[vulkan][GPU][raytracing]")
{
	...
	spall::Resource<spall::IShader> rayGenShader = device.pipelines().createShader(
		spall::ShaderStage::RayGeneration,
		RayTracingPipelineProceduralLibrarySpirv);
	spall::Resource<spall::IShader> missShader = device.pipelines().createShader(
		spall::ShaderStage::Miss,
		RayTracingPipelineProceduralLibrarySpirv);
	spall::Resource<spall::IShader> closestHitShader = device.pipelines().createShader(
	...
	"[vulkan][GPU][raytracing]")
{
	...
	const std::array<RayResult, 2> results = traceScene(
		...
		scene,
		false,
		RayQueryProceduralComputeSpirv);
